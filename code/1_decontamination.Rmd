---
title: "Secondary QC and Decontamination - NBS2021 & NBS/SBS 2022 eDNA w/ MiFish"
...
Inputs: This code starts with the ASV table output from dada_blast.sh that uses dadasnake for preliminary quality control.
...
```{r}
#asv_table <- readRDS("data/filtered.seqTab.RDS") %>%
asv_table <- readRDS("/home/kimberly.ledger/BeringSea_Arctic_eDNA/data/filtered.seqTab.RDS") %>%
  select(!Row.names)
...
#metadata <- read.csv("data/NBS_SBS_2022_metadata.csv")
metadata <- read.csv("/home/kimberly.ledger/BeringSea_Arctic_eDNA/data/NBS_SBS_2022_metadata.csv")
...
asv_table_with_sample_type <- metadata %>%
  dplyr::select(Sample_ID, sample_type, collection_year, project, extraction_date) %>%
...
okay, so ASV8 must be the sturgeon and very few reads from other ASVs showed up in the positive controls. little to no tag-jumping in this dataset. 
...
asv_table_with_sample_type %>%
  ...
  ggplot(aes(x=Sample_ID, y=reads, fill=ASV)) +
  facet_grid(~extraction_date, scales = "free_x") + 
  geom_bar(stat = "identity") + 
  theme_bw() +
  labs(
    y = "number of sequencing reads",
