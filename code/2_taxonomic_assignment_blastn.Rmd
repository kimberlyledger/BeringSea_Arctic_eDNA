...
```{r}
#write.csv(filtered_tax_df, "/home/kimberly.ledger/BeringSea_Arctic_eDNA/outputs/tax_df_20240703.csv")
```
...
gadidae_custom <- read.csv("/home/kimberly.ledger/BeringSea_Arctic_eDNA/outputs/gadidae_asv_id.csv") %>%
  select(!X) %>%
  mutate(taxon = ifelse(taxon == "Gadid", "Gadus", taxon))  ### whoops but gadid instead of gadus in table
