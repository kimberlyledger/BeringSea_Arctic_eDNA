...
metadata_mini <- metadata %>%
  select(extraction_ID, project, collection_year, location1, depth, longitude, latitude)  ## this set up doesn't work for DBO sample format right now - will need to fix 
```
...
```{r}
bottles_lessthan_2500 <- reads_per_bottle%>%
  filter(sample_type == "sample") %>%
  filter(tot_reads < 2500) %>%
  left_join(metadata_mini)
```
...
#NBS_2023 <- asv_table_filter5 %>%
 # filter(project_year == "NBS_2023")

NBS_2023 <- taxon_table_filter7 %>%
  filter(project_year == "NBS_2023")
```
...
taxon_long <- taxon_table %>%
  ...
  pivot_longer(cols = c(10:121), names_to = "taxon", values_to = "reads") %>%    #### CHECK COLUMN VALUES!!!
  filter(reads > 0) %>% # filter out entries with no reads
  #arrange(sample_ID_date, desc(reads))
  arrange(extraction_ID, desc(reads))  # sort reads in descending order and group by sample

st <- taxon_long %>%
  filter(location1 %in% c("32"))
...
#out_nbs2023_st23 <- iNEXT(read_lists_vector, q = 0, datatype = "abundance")
...
#out_nbs2023_st12_23_38_43 <- iNEXT(read_lists_vector, q = 0, datatype = "abundance") 
...
out_nbs2021_st32_m <- iNEXT(read_lists_vector, q = 0, datatype = "abundance", size = m)


## add a max endpoint ? say endpoint = 60000
```
...
```{r}
df <- fortify(out_nbs2021_st32_m, type=1)
...
when working at the bottle level, it takes far more reads to reach a plateau... maybe around 2500 reads? 

or around where richness reaches 95% (?) of max value. 


right now i am visually picking a possible threshold. but what about a quantitative method? 

note:  nbs2023 station 38 confuses me... 
...
accumulation_data <- out_nbs2023_st12_23_38_43$iNextEst[[1]] 
accumulation_data <- out_nbs2023_st2_80knots$iNextEst[[1]] 
accumulation_data <- out_nbs2021_st46_m$iNextEst[[1]] 

%>%
  filter(Assemblage == "e03351")

# Load required library for non-linear modeling
library(nls2)

# Fit a model (logistic growth model is often used)
logistic_model <- nls(qD ~ SSlogis(m, Asym, xmid, scal),
                      data = accumulation_data)

# Summary of the model
summary(logistic_model)

# Predict species richness using the fitted model
predicted_richness <- predict(logistic_model, newdata = accumulation_data)

# Plot the accumulation curve with the fitted model
ggplot(accumulation_data, aes(x = m, y = qD)) +
  geom_point() +
  geom_line(aes(y = predicted_richness), color = "blue") +
  labs(x = "Sampling Effort", y = "Species Richness",
       title = "Species Accumulation Curve with Fitted Logistic Model")

```


okay, so for many samples the first non-1 m, the species richness reaches it's plateau.  
...
ALRIGHT - i don't think after decontamination my analyses rely on unique pcr replicate.  let me combine data for each bottle of water and retry out some of these analyses.  

```{r}
asv_table_filter7 <- asv_table_filter5 %>% 
  group_by(extraction_ID, seq_date, project_year, project, collection_year, sample_type, ASV, taxon, taxonomic_level) %>%
  summarize(tot_reads = sum(reads))

taxon_table_filter7 <- asv_table_filter7 %>%
  group_by(extraction_ID, seq_date, project_year, project, collection_year, sample_type, taxon, taxonomic_level) %>%
...
```{r}
reads_per_bottle <- taxon_table_filter7 %>%
  group_by(extraction_ID, sample_type) %>%
  summarise(tot_reads = sum(reads))   ##570

bottle_atleast_2500 <- reads_per_bottle %>%
  filter(tot_reads >= 2500)            ##485
```

now that i have a filtered dataframe, let me plot some relative abundance barplots for sampling stations 

```{r}
taxon_table_filter8 <- taxon_table_filter7 %>%
  filter(extraction_ID %in% bottle_atleast_2500$extraction_ID) %>%
  left_join(metadata_mini)
```
