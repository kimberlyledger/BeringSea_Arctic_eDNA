...
```{r}
oncor <- read.table("/home/kimberly.ledger/BeringSea_Arctic_eDNA/outputs/oncorhynchus_results.txt")  %>%
...
```{r}
gadidae <- read.table("/home/kimberly.ledger/BeringSea_Arctic_eDNA/outputs/gadidae_results.txt")  %>%
  filter(V4 > 100) %>% # make sure all retained matches are >100 bp
  select(-V5, -V6, -V7, -V8, -V9, -V10, -V11, -V12, -V13, -V14) %>%  #remove unnecessary columns
  group_by(V1, V2) %>% # group by the sequence key and the full taxonomy to reduce duplicate entries
  unique() %>% # doing that reduced the number of entries
...
#export 
write.csv(gadidae_df, "/home/kimberly.ledger/BeringSea_Arctic_eDNA/outputs/gadidae_asv_id.csv")
```
